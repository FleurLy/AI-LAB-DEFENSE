...
def test_openrouter_errors_do_not_expose_secrets(
    ...
    for name in ("llm", "jev"):
        assert response.json()[name]["status"] == "error"
        assert response.json()[name]["error"]["code"] == expected_code
        assert response.json()[name]["error"]["status_code"] == expected_status
    for sensitive in (
...
def test_adapter_value_error_does_not_expose_raw_response(
    ...
    assert response.status_code == 200
    for name in ("llm", "jev"):
        assert response.json()[name]["error"]["code"] == "provider_error"
        assert response.json()[name]["error"]["status_code"] == 502
